...
def ErrorMessage(message):
    ...
    try:
        # Try wx first
        import wx
        from Mopy.src.balt import ShowError
...
def VerifyRequirements():
    ...
    if sys.version_info[0:2] != (3, 6):
        ...
        errors.append(
            (_(
                'Python 3.6.3 is required.  Current version is %(version)s.  Get it from:')
    ...
    try:
        ...
        if version < 218:
            errors.append(
                (_(
                    'pywin32 218 is required.  Current version is %(version)s.  Get it from:')
                 % {'version': version})
                + '\n    %s' % url)
    except ImportError:
        errors.append(
            _('pywin32 218 is required.  None detected.  Get it from:')
            + '\n    %s' % url)

    if errors:
        msg = (_(
            'Cannot start Wrye Bash!  Please ensure that Python dependencies are installed correctly.')
               + '\n\n' +
               '\n\n'.join(errors))
        ErrorMessage(msg)
        return False
...
def main():
    try:
        ...
        # --Setup translations
        try:
            from Mopy.src.bolt import Translations
        ...
        from Mopy.src.bolt import OneInstanceChecker
